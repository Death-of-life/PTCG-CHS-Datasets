...
const REMOTE = process.env.PTCG_SYNC_REMOTE ?? "upstream";
const BRANCH = process.env.PTCG_SYNC_BRANCH ?? "main";
const TARGETS = ["ptcg_chs_infos.json"];
...
async function main() {
  ...
  if (!hasTargetedChanges(remoteRef)) {
    console.log("No upstream JSON changes detected.");
    return;
  }

  if (hasLocalTargetedChanges()) {
    throw new Error(
      "Local changes exist in ptcg_chs_infos.json. Please commit/stash them before syncing upstream data.",
    );
  }

  console.log(`Updating ${TARGETS.join(", ")} from ${remoteRef} ...`);
  git(["restore", "--source", remoteRef, "--worktree", "--", ...TARGETS]);
